...
- v2.0.6 (22.12.02) : 출석부를 수정하고 구조개선을 했습니다.
    ...
    2. TypeScript답게 코드에 타입을 입혀야할 필요성을 느껴서 타입을 적용했고 리펙터링을 진행하여 학생 명단을 내려줄 때 효율적인 로직을 가지도록 수정했습니다.
...
4. DTO를 사용해서 클라이언트와 서버의 의존성을 줄여야 합니다. 현재는 서버의 Entity를 그대로 던져주고 있는데, Entity가 변경되면 클라이언트는 그에 따라 변경을 해야합니다. 이는 올바르지않습니다. DTO를 도입하여 클라이언트가 Entity에 의존하는 것이 아닌, DTO에 의존하도록 변경해야 합니다.
