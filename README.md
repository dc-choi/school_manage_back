...
4. 자동으로 조 편성을 해주는 기능을 추가해야 할 거 같습니다. 중고등부는 초등부와 다르게 나이에 상관없이 조별로 활동하기 때문입니다.
5. 출석부 입력시 이름 고정 및 아이들의 나이를 추가해야 합니다. 사용시 불편함을 느꼈다고 합니다.
6. 출석 입력을 select로 변경해야 합니다. 피드백을 주셨습니다. (select시 테이블이 깨지는 문제 해결하기)
7. 폰트가 이쁘지않다는 피드백을 받아서 폰트 변경이 필요합니다.
8. 축일을 저장하도록 수정해야 합니다. 축일을 저장할 수 있도록 해달라는 초등부의 요청을 받았습니다.
